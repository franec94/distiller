...
def calculate_uniformed_baseline_csv_out_file(bsln_uniform_df: pd.DataFrame, conf_dict: dict) -> pd.DataFrame:
    """Compute, save and retrieve updated and uniformed dataframe version of input provided dataframe.
    Args:
    -----
    `bsln_uniform_df` - pd.DataFrame.\n
    `conf_dict` - dict object with conf data.\n
    Return:
    -------
    `bsln_uniform_df` -  pd.DataFrame, updated instance.\n
    """
    ...
    for old_col_name, new_col_name in zip(rename_columns, renamed_columns):
        if old_col_name not in bsln_uniform_df.columns: continue
        bsln_uniform_df[f"{new_col_name}"] = bsln_uniform_df[f"{old_col_name}"].values
        bsln_uniform_df = bsln_uniform_df.drop([f"{old_col_name}"], axis = 1)
        pass

    bsln_uniform_df["size(byte)"] = bsln_uniform_df["size(byte)"] / 8
    bsln_uniform_df["footprint(%)"] = [100.] * bsln_uniform_df.shape[0]

    def create_compression_class_column(item):
        compression, hl, hf = item
        # return f"{compression.upper()}:hf={int(hf)}:hl={int(hl)}"
        return f"Baseline:hf={int(hf)}:hl={int(hl)}"

    drop_columns = "hl,hf,width,seed,eta,compression,#params".split(",")
    bsln_uniform_df["cmprss-class"] = list(map(create_compression_class_column, bsln_uniform_df[[f"compression", f"hl", f"hf"]].values))
    ...
    def create_compression_class_2(cmprss_class):
        ...
        if cmprss_class.lower().startswith("Baseline".lower()):
            return "BASELINE" 
        return cmprss_class
    bsln_uniform_df["cmprss-class-2"] = list(map(create_compression_class_2, bsln_uniform_df[f"cmprss-class"].values))

    bsln_uniform_df['date'] = list(bsln_uniform_df.index)
    bsln_uniform_df['init-from'] = ['-'] * bsln_uniform_df.shape[0]
    columns_order = "date,init-from,size(byte),footprint(%),psnr,bpp,CR,mse,ssim,cmprss-class,cmprss-class-2".split(",")
    bsln_uniform_df = bsln_uniform_df[columns_order]

    bsln_uniform_csv_path = os.path.join(f"{uniform_csv_dir}", "train_uniform_baselines.csv")
    bsln_uniform_df.to_csv(bsln_uniform_csv_path)
    # bsln_uniform_df.head(5)
    print(f"Results saved to: {bsln_uniform_csv_path}")
    return bsln_uniform_df
...
def main(args):
    ...
    print('Load baseline data to be uniformed...')
    bsln_df = load_data_baseline(im_cropped)
    bsln_uniform_df = copy.deepcopy(bsln_df)
    # bsln_uniform_df.head(5)

    print('Save results as uniformed csv structure...')
    bsln_uniform_df = calculate_uniformed_baseline_csv_out_file(bsln_uniform_df, conf_dict)
    print(bsln_uniform_df.head(3))
    ...
    if conf_dict['save_to_cmprs_dataset']:
        ...
        if not check_file_exists(cmprs_df_csv_path, False):
            ...
            cmprs_df = copy.deepcopy(bsln_uniform_df)
            ...
            print('RAW DF:', bsln_uniform_df.shape)
            
            list_dfs = [cmprs_df, bsln_uniform_df]
