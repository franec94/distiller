...
def load_jpeg_dataset_as_dataframe_container() -> pd.DataFrame:
    jpeg_df, _, _, _= get_some_dfs()
    return jpeg_df


def load_jpeg_dataset(dtype: str = "bunch") -> object:
    """Load Jpeg dataset.
    Returns:
    --------
    ...
    if dtype.lower() == "dataframe".lower():
        ...
        jpeg_dataset = load_jpeg_dataset_as_dataframe_container()
    return jpeg_dataset
...
def load_pruning_dataset_as_dataframe_container() -> pd.DataFrame:
    _, _, _, pruning_df = get_some_dfs()
    return pruning_df


def load_prunining_dataset(dtype: str = "bunch") -> object:
    """Load Jpeg dataset.
    Returns:
    --------
    ...
    if dtype.lower() == "dataframe".lower():
        # raise Exception(f"Warning: provided dtype '{dtype}' not yet implemented!")
        pruning_dataset = load_pruning_dataset_as_dataframe_container()
    return pruning_dataset
