...
from src.qat_features.create_comparing_qat_tables import create_comparing_results_table
from src.pruned_models_utils.pruned_models_utils import get_list_of_pruned_datasets, get_selected_dataset
...
ROOT_DIR = "/home/franec94/Documents/thesys-siren/codebase/results/datasets"
...
def pick_tmp_dataset_wrapper(dataset_path_wg):
    """
    TODO - comment it: pick_tmp_dataset_wrapper
    """
    all_merged_csv_list, merged_csv_dict = get_list_of_pruned_datasets()
    @interact
    def pick_tmp_dataset(datasets_list=all_merged_csv_list):

        if datasets_list.lower() == 'All'.lower():
            a_dataset_path = ROOT_DIR
        else:
            a_dataset_path = merged_csv_dict[datasets_list]
...
def show_hist_or_pie_quant_tech(a_df: pd.DataFrame):
    """
    TODO - comment it: show_hist_or_pie_quant_tech
    """
    ...
    def scatter_plot(hist_or_pie_chart=True,
        # attribute=list(a_df.select_dtypes('number').columns), theme=list(cf.themes.THEMES.keys()),
...
def show_scatter_plot(a_df: pd.DataFrame, jpeg_df: pd.DataFrame, optional_dfs_dict: dict = None, target_class:str = 'cmprss-class-2'):
    """
    TODO - comment it: show_scatter_plot
    """
...
def show_scatter_plot_2(a_df: pd.DataFrame, jpeg_df: pd.DataFrame, optional_dfs_dict: dict = None, target_class:str = 'cmprss-class-3'):
    @interact
    def scatter_plot(
        flag_add_jpeg_data=False, flag_add_siren_data=False,
        x=list(a_df.select_dtypes('number').columns),
...
def show_box_plot(a_df:pd.DataFrame, jpeg_df:pd.DataFrame = None, optional_dfs_dict: dict = None, target_class:str = 'cmprss-class-2'):
    """
    TODO - comment it: show_box_plot
    """
...
def show_box_plot_2(a_df:pd.DataFrame, jpeg_df:pd.DataFrame = None, optional_dfs_dict:dict = None, target_class:str = 'cmprss-class-3'):
    # qat_df[['cmprss-class-3', 'psnr']].pivot(columns='cmprss-class-3', values='psnr').iplot(kind='box')
    @interact
    def scatter_plot(flag_add_jpeg_data=False, flag_add_siren_data=False,
                     attribute=list(a_df.select_dtypes('number').columns),
...
def create_resulting_table_for_comparison_reasons(table_df:pd.DataFrame):
    """
    TODO - comment it: create_resulting_table_for_comparison_reasons
    """
    metadata_table = dict(
        table_df=table_df,
...
def get_best_siren_pruned_rows_by_target_as_df(cmprss_df:pd.DataFrame, a_class:str, best_attr:str = "psnr"):
    """
    TODO - comment it:  get_best_siren_pruned_rows_by_target_as_df
    """
    list_df = []
...
def get_best_jpeg_lower_equal_than_pruned_models(jpeg_df:pd.DataFrame, siren_prune_best_df:pd.DataFrame, best_attr:str = "psnr") -> pd.DataFrame:
    """
    TODO - comment it: get_best_jpeg_lower_equal_than_pruned_models
    """
...
def create_resulting_table_for_comparison_reasons_2(baseline_data_df:pd.DataFrame, cmprss_df:pd.DataFrame, jpeg_df:pd.DataFrame):
    """
    TODO - comment it: create_resulting_table_for_comparison_reasons_2
    """
