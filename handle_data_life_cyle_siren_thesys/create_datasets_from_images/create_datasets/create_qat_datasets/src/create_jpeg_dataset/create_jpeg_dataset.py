...
# Utils Functions
# =========================================================== #
def get_histogram_image(image: PIL.Image):
    """Get histogram image.
    Args:
    -----
    `iamge` - PIL.Image.\n
    ...
    plt.title(f"Histogram Camera Image({image.size[0]}x{image.size[1]})")
    return fig
...
def get_jpeg_namedtuple_builder() -> object:
...
def calculate_cropped_image_size(image, quality) -> (PIL.Image, float):
...
def calculate_scores(image, im_jpeg, image_dim_bits, compressed_file_size_bits, quality) -> list:
    """Calculate scores.
    Returns:
    --------
    `values` - list object.\n
...
def calculate_several_jpeg_compression(image: PIL.Image, image_dim_bits:int, qualities:list) -> (list, list):
    ...
    WeightsPsnr = get_jpeg_namedtuple_builder()
    ...
    with tqdm.tqdm(total=len(qualities)) as pbar:
