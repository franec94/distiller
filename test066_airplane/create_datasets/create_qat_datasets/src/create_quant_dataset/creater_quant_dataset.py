...
def adjust_out_dataframe_by_conf_data(conf_data: dict, a_df: pd.DataFrame, n_hf=64, n_hl=5) -> pd.DataFrame:
    ...
    if pruned_test066_df.shape[0] == 0: return adjusted_df
    if not date_str in pruned_test066_df["date_train"].unique(): return adjusted_df

    # pick_cols = "size_byte_th,prune_rate,prune_techs".split(",")
    pos = pruned_test066_df["date_train"] == date_str
    a_row = pruned_test066_df[pos].head(1)
